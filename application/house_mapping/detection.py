...
def get_masks(block_id, mask_list):
...
def ensure_geometry(geometry):
    ...
    if not geometry.IsValid():
        print(colored('use simplify to fix geometry', 'blue'))
        geometry = geometry.SimplifyPreserveTopology(simplify_tolerance)
...
with GDALDataset(os.path.join(work_space, 'image/honghu.tif')) as dataset:
    ...
    for batch_bounds in tqdm.tqdm(bounds):
        ...
        for index, result in enumerate(results):
            ...
            pre_masks = []
            get_masks('{}-{}'.format(row - bound_offset_row, column - bound_offset_column), pre_masks)
            get_masks('{}-{}'.format(row, column - bound_offset_column), pre_masks)
            get_masks('{}-{}'.format(row - bound_offset_row, column), pre_masks)
            get_masks('{}-{}'.format(row - bound_offset_row, column + bound_offset_column), pre_masks)
